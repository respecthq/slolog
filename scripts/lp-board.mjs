...
for (const f of readdirSync(dir).filter((x) => x.endsWith('.md') && !x.startsWith('_'))) {
  ...
  machines.push({ slug, fresh, factRefs, name: one('name'), maker: one('maker'), rel, source: one('source'), spec: one('specSource'), draft: flag('draft'), wait, refs, ceiling: one('ceiling'), noCeiling: flag('noCeiling'),
...
const rowsWait = waiting.map((m) => `<tr><td><b>${esc(m.name)}</b>${fchip(m.fresh)}<div class="sub">${esc(m.maker)}</div>${fnote(m.fresh)}</td><td class="num">${esc(m.rel || '未定')}</td><td>${m.wait.map(chip).join(' ')}${factLinks(m.factRefs) ? `<div class="sub">確認元：${factLinks(m.factRefs)}</div>` : ''}</td><td>${m.draft ? '<span class="mute">非公開</span>' : link(SITE + m.slug + '/', 'LP')} · ${link(m.source, '出典')}${m.spec ? ' · ' + link(m.spec, 'スペック') : ''}</td></tr>`).join('');
const rowsDone = done.map((m) => `<tr><td>${esc(m.name)}${fchip(m.fresh)}<div class="sub">${esc(m.maker)}</div>${fnote(m.fresh)}</td><td class="num">${esc(m.rel)}</td><td class="sub">${esc(m.note || '')}${m.note && factLinks(m.factRefs) ? '<br>' : ''}${factLinks(m.factRefs)}</td><td>${link(SITE + m.slug + '/', 'LP')} · ${link(m.source, '出典')}${m.spec ? ' · ' + link(m.spec, 'スペック') : ''}</td></tr>`).join('');
